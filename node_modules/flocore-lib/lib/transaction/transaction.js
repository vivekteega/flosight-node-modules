...
Transaction.MAX_MONEY = 160000000 * 1e8; // Flo has 160M coin cap
...
Transaction.prototype.toBuffer = function(noWitness, noFloData) {
  var writer = new BufferWriter();
  return this.toBufferWriter(writer, noWitness, noFloData).toBuffer();
};
...
Transaction.prototype.toBufferWriter = function(writer, noWitness, noFloData) {
  ...
  if (this.version >= 2 && noFloData !== true){
    let floDataBuffer = Buffer.from(this.strFloData)
    writer.writeVarintNum(floDataBuffer.length)
    writer.write(floDataBuffer);
  }
  return writer;
};
...
Transaction.prototype.fromBufferReader = function(reader) {
  ...
  if (this.version >= 2){
    var size = reader.readVarintNum();
    var dataStrBuffer = reader.read(size);
    var dataStr = Buffer.from(dataStrBuffer).toString();
    this.strFloData = dataStr;
  }
...
Transaction.prototype.fromObject = function fromObject(arg) {
  ...
  if (transaction.floData){
    this.strFloData = transaction.floData;
  }
...
Transaction.prototype.getSignatures = function(privKey, sigtype) {
  ...
  sigtype = sigtype || Signature.SIGHASH_ALL || Signature.SIGHASH_OMIT_FLO_DATA;
...
Transaction.prototype.verify = function() {
  ...
  for (var i = 0; i < this.outputs.length; i++) {
    ...
    if (txout._satoshisBN.gt(new BN(Transaction.MAX_MONEY.toString(16), "hex"))) {
      return 'transaction txout ' + i + ' greater than MAX_MONEY';
    }
    valueoutbn = valueoutbn.add(txout._satoshisBN);
    if (valueoutbn.gt(new BN(Transaction.MAX_MONEY.toString(16), "hex"))) {
