# Transaction
Flocore provides a very simple API for creating transactions. We expect this API to be accessible for developers without knowing the working internals of florincoin in deep detail. What follows is a small introduction to transactions with some basic knowledge required to use this API.
...
Note also that an output can be used only once. That's why there's a concept of "change address" in the florincoin ecosystem: if an output of 10 FLO is available for me to spend, but I only need to transmit 1 FLO, I'll create a transaction with two outputs, one with 1 FLO that I want to spend, and the other with 9 FLO to a change address, so I can spend this 9 FLO with another private key that I own.
...
var transaction = new Transaction().fee(1e8);  // Generous fee of 1 FLO
```
...
- `sequenceNumber`: a `number`, the sequence number, see [florincoin's developer guide on nLockTime and the sequence number](https://florincoin.org/en/developer-guide#locktime-and-sequence-number).
...
- `getSignatures`: takes an array of `PrivateKey` or strings from which a `PrivateKey` can be instantiated; the transaction to be signed; the kind of [signature hash to use](https://florincoin.org/en/developer-guide#signature-hash-types). Returns an array of objects with the following properties:
  - `signature`: an instance of [Signature](https://github.com/oipwg/flocore/blob/master/lib/crypto/signature.js)
...
- `toString` or `uncheckedSerialize`: Returns an hexadecimal serialization of the transaction, in the [serialization format for florincoin](https://florincoin.org/en/developer-reference#raw-transaction-format).
...
## Upcoming changes
We're debating an API for Merge Avoidance, CoinJoin, Smart contracts, CoinSwap, and Stealth Addresses. We're expecting to have all of them by some time in 2015. Payment channel creation is available in the [flocore-channel](https://github.com/oipwg/flocore-channel) module.
